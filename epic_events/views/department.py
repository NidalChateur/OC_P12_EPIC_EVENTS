...
class ListView(crud_permission, SearchPostMixin):
...
class SearchView(crud_permission, SearchPostMixin):
...
class DetailView(crud_permission, View):
...
class CreateView(crud_permission, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            qs = model.objects.filter(slug__contains=slugify(form.cleaned_data["name"]))
            if qs:
                obj = qs[0]
                messages.info(
                    request,
